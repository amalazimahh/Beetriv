...
//echo $email
// $id = $_GET['product'];
$bidding = $conn->query("SELECT * FROM product_bid WHERE bid_result = '$email'");
$bid = $bidding->fetch(PDO::FETCH_ASSOC);

$prd_id = $bid['prd_id'];
$prd_price = $bid['current_bid'];

$result = $conn->query("SELECT * FROM product WHERE prd_id = '$prd_id'");
...
$stmt = $conn->query("SELECT * FROM order_details WHERE email = '$email'");
$res = $stmt->fetch(PDO::FETCH_ASSOC);

// if (empty($res['payment_stat']) && prd) {
$select = "SELECT * FROM order_details WHERE 1";
$insert = $conn->query ("INSERT INTO order_details (prd_id, prd_name, prd_price, prd_qty, user_id, email, username, stat, contact_no, payment_mthd, payment_stat) VALUES ('$prd_id','$prd_name','$prd_price','$prd_qty','$user_id','$email','$username','$status','$contact_no','$payment_mthd','$paymentStat')");
// }
