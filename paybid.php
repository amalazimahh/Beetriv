  ...
  <nav class="navbar sticky-top navbar-expand-lg navbar-light bg-light" style='color:black' > 
            <div class="container px-4 px-lg-5">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <a class="nav-item nav-link" style='color:black' aria-current="page" href="user-profile.php"><i class="bi-person-circle"></i></a></li>
                    <form action = "product-details.php" method = "post">
                    <a type="submit" name="logout" class="nav-item nav-link" style='color:black' aria-current="page" href="login.php"><i class="bi bi-box-arrow-right"></i></a>
                    </form>
                    </a>
                    </ul>
                </div>
           ...
           <thead>
            ...
            <tbody>
                <?php 
                    ...
                    $stmt = $conn->query("SELECT * FROM product_bid WHERE bid_result = '$email'");
                    $result = $stmt->fetch(PDO::FETCH_ASSOC);
                    $prd_id = $result['prd_id'];
                    $bid = $conn->query("SELECT * FROM product WHERE prd_id = '$prd_id'");
                    $won = $bid->fetch(PDO::FETCH_ASSOC);

                    ?>
                    <tr>
                        <td>
                            <img src="data:image/jpg;charset=utf8;base64,<?php echo base64_encode($won['prd_img']); ?>"class="rounded img-thumbnail mr-2" style="width:40px;">
                            <?php echo $won['prd_name'];?>               
                        </td>
                        <td>
                            [Vendors]
                        </td>
                        <td>
                            $<?php echo $result['current_bid'];?>
                        </td>
                        <td>
                            <?php echo "1";?>
                        </td>
                ...
                <tr class="border-top border-bottom">
                    ...
                    <td><strong>$<?php echo $result['current_bid'];?></strong></td>
                </tr> 
                </tr>
            </tbody> 
        </table>
